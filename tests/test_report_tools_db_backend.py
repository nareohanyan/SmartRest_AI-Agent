...
def test_resolve_scope_db_with_fallback_uses_mock_when_db_unavailable(
    ...
    monkeypatch.setenv("SMARTREST_SCOPE_BACKEND_MODE", "db_with_fallback")
    ...
    assert response.canonical_profile_id is None
    assert response.canonical_user_id is None
...
def test_run_report_db_fallback_warns(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("SMARTREST_REPORT_BACKEND_MODE", "db_with_fallback")

    def _raise_backend(*_args: object, **_kwargs: object) -> object:
        raise report_tools.SmartRestReportBackendUnsupportedError("unsupported")

    monkeypatch.setattr(report_tools, "run_smartrest_report", _raise_backend)
    response = report_tools.run_report_tool(_report_request())

    assert "smartrest_backend_fallback_to_mock" in response.warnings
    assert "mock_backend_deterministic_data" in response.warnings
...
def test_run_report_db_strict_raises(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    with pytest.raises(report_tools.SmartRestReportBackendUnsupportedError):
