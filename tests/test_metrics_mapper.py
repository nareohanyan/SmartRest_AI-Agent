...
from app.agent.report_tools import run_report_tool
...
def test_map_sales_by_source_report_to_base_metrics_with_total() -> None:
    ...
    assert mapped["day_count"] == Decimal("7")
    assert mapped["source_glovo_sales"] == Decimal("4100.0")
    assert mapped["source_wolt_sales"] == Decimal("2200.0")
    assert mapped["source_takeaway_sales"] == Decimal("845.67")
    assert mapped["source_in_store_sales"] == Decimal("5200.0")
    assert mapped["sales_total"] == Decimal("12345.67")
